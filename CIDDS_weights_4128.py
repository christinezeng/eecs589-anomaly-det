...
df = pd.read_csv("samples_all_CIDDS_4128.csv")
...
print("LOOK HERE!", df.head(5))


labels = ['benign', 'bruteForce', 'dos', 'pingScan', 'portScan']
torch_count_labels = torch.tensor(count_labels.values, dtype=torch.float32).cuda()
class_weights = 1.0 / (torch_count_labels + 1e-6)
...
features_flow = ['daytime', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Duration', 'Packets', 'Bytes', 'ACK', 'PSH', 'RST', 'SYN', 'FIN', 'ICMP ', 'TCP  ', 'UDP  ']
...
def create_dataloader(df, subgraph_size=80):
    data = []
...
model = HeteroGNN(dim_h=64, dim_out=5, num_layers=6).to(device)
...
def test(loader):
    ...
    for batch in loader:
        ...
        loss = F.cross_entropy(out, batch['flow'].y.float(), weight=class_weights)
        print("LOOK HERE:", out.argmax(dim=1))
